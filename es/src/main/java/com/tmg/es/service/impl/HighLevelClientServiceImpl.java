...
import com.tmg.es.service.HighLevelClientService;
   ...
   @PostConstruct
    ...
    public boolean deleteIndex(BaseIndex baseIndex) throws IOException {
        if (baseIndex != null && !StringUtils.isTrimEmpty(baseIndex.getIndex())) {
            ...
            throw new IllegalArgumentException("delete index fail , index can not be null");
        }
    }
    ...
    public boolean openIndex(BaseIndex baseIndex) throws IOException {
        if (baseIndex != null && !StringUtils.isTrimEmpty(baseIndex.getIndex())) {
            ...
            throw new IllegalArgumentException("open index fail , index can not be null");
        }
    }
    ...
    public boolean closeIndex(BaseIndex baseIndex) throws IOException {
        if (baseIndex != null && !StringUtils.isTrimEmpty(baseIndex.getIndex())) {
            ...
            throw new IllegalArgumentException("close index fail , index can not be null");
        }
    }
    ...
    public boolean indexOrDocumentIsExists(String index, String type, String documentId) throws IOException {
        int code = 200;
        String endpoint = "/" + index + "/" + type + "/" + encodingEsId(documentId);
    ...
    public boolean delete(String index, String type, String id) throws IOException {
        String endpoint = "/" + index + "/" + type + "/" + encodingEsId(id);
    ...
    public void updateByQueryDSL(String index, Map<String, Object> queryDSL) throws IOException {
        ...
        HttpEntity entity = new NStringEntity(source, ContentType.APPLICATION_JSON);
        Response response = restClient.performRequest(POST, "/" + index + "/_update_by_query?conflicts=proceed" , Collections.<String, String>emptyMap(), entity);
    ...
    public List<Map<String, Object>> matchAllQuery(String index, String type, String routing) throws IOException {
       if(!indexOrDocumentIsExists(index,null,null)){
           throw new IllegalArgumentException("index:"+index+" is missing");
       }
    ...
    public List<Map<String, Object>> multiMatchQuery​(String index, Object text, Set<String> fileNames) throws IOException {
        if(fileNames.isEmpty()){
            throw new IllegalArgumentException("The field names is null");
    ...
    public List<Map<String, Object>> termsQuery(String index, String fileName, Set<String> values) throws IOException {
       if(values.isEmpty()){
           throw new IllegalArgumentException("the terms is null");
       }
    ...
    private static String encodingEsId(String id) throws UnsupportedEncodingException {
       return  id!=null?URLEncoder.encode(id, "UTF-8").replace("+","%20"):null;
    }

    @Override
    public List<Map<String,Object>> multiSearchWithTemplateId(String indexName,String templateId,List<Map<String,Object>> params) throws IOException{

        if(indexName==null || templateId==null || params==null || params.size()==0){
            throw new IllegalArgumentException("index name , template id and params must provide");
        }
        int size = params.size();
        List<String> indexNames = new ArrayList<>(size);
        List<String> templateIds = new ArrayList<>(size);
        for(int i=0;i<size;i++){
            indexNames.add(indexName);
            templateIds.add(templateId);
        }

        return multiSearchWithTemplateId(indexNames, templateIds, params);
    }

    @Override
    public List<Map<String,Object>> multiSearchWithTemplateId(String indexName,List<String> templateIds,List<Map<String,Object>> params) throws IOException{

        if(indexName==null || templateIds==null || templateIds.size()==0 || params==null || params.size()==0){
            throw new IllegalArgumentException("index name , template ids and params must provide");
        }
        if(templateIds.size()!=params.size()){
            throw new IllegalArgumentException("template templateIds and params must be the same size");
        }
    ...
    /**
     * 使用已存储的模板进行批量查询
     * @param indexNames 索引名称序列
     * @param templateIds 依次对应各索引的模板名称序列
     * @param params 依次对应各模板的参数序列
     * @return
     */
    @Override
    public List<Map<String,Object>> multiSearchWithTemplateId(List<String> indexNames,List<String> templateIds,List<Map<String,Object>> params) throws IOException{
        return multiSearchWithTemplate(indexNames,templateIds,params, ScriptType.STORED);
    }


    @Override
    public List<Map<String,Object>> multiSearchWithTemplateSource(String indexName,String source,List<Map<String,Object>> params) throws IOException{

        if(indexName==null || source==null || params==null || params.size()==0){
            throw new IllegalArgumentException("index name , template source and params must provide");
        }
        int size = params.size();
        List<String> indexNames = new ArrayList<>(size);
        ...
        return multiSearchWithTemplateSource(indexNames, sources, params);
    }

    @Override
    public List<Map<String,Object>> multiSearchWithTemplateSource(String indexName,List<String> sources,List<Map<String,Object>> params) throws IOException{

        if(indexName==null || sources==null || sources.size()==0 || params==null || params.size()==0){
            throw new IllegalArgumentException("index name , template sources and params must provide");
        }
        if(sources.size()!=params.size()){
            throw new IllegalArgumentException("template sources and params must be the same size");
        }
        ...
        return multiSearchWithTemplateId(indexNames, sources, params);
    }

    @Override
    public List<Map<String,Object>> multiSearchWithTemplateSource(List<String> indexNames,List<String> sources,List<Map<String,Object>> params) throws IOException{
        return multiSearchWithTemplate(indexNames,sources,params, ScriptType.STORED);
    }

    /**
     * 使用模板进行批量查询
     * @param indexNames 索引名称序列
     * @param scripts 依次对应各索引的模板source内容或名称序列
     * @param params 依次对应各模板的批量查询参数序列
     * @return
     * @throws IOException
     */
    protected List<Map<String,Object>> multiSearchWithTemplate(List<String> indexNames,List<String> scripts,List<Map<String,Object>> params,ScriptType type) throws IOException{

        if(indexNames==null || indexNames.size()==0 || scripts==null || scripts.size()==0 || params==null || params.size()==0){
            throw new IllegalArgumentException("index names , template scripts(id or source) and params must provide");
        }
        if(indexNames.size()!=params.size() || scripts.size()!=params.size()){
            throw new IllegalArgumentException("index names , template scripts(id or source) and params must be the same size");
        }
        /****** use Restful request to perform multiSearchTemplate ****************************/

        String endpoint = "/_msearch/template";
        ...
        for(int i=0;i<params.size();i++) {
            ...
            _paramMap.clear();
            _paramMap.put("index",indexNames.get(i));
            query.append(JsonUtils.toJSONString(_paramMap)).append("\n");

            _paramMap.clear();
            _paramMap.put(type.getParseField().getPreferredName(),scripts.get(i));
            _paramMap.put("params", params.get(i));
            query.append(JsonUtils.toJSONString(_paramMap)).append("\n");

            /*
             * like multisearch below
              * GET _msearch/template
              * {"index" : "main"}
              * { "source": "{\"query\":{...}}", "params": { "query_string": "some message" } }
              * {"index" : "main"}
              * { "source": "{\"query\":{...}}", "params": { "field": "user", "value": "test" } }
              *
              * or
              *
              * {"index" : "main"}
              * { "id": "template_name", "params": { "query_string": "some message" } }
              * {"index" : "main"}
              * { "source": "template_name", "params": { "field": "user", "value": "test" } }
             */
        }
        ...
        try {
            ...
            for(int i=0;i<responses.size();i++){
                JSONObject _obj = responses.getJSONObject(i);
                list.add(_obj.getJSONObject("hits"));
            }
            return list;
        } catch (Exception e) {
            throw e;
